...
def synthLe : (s : Nat) → {S : Fset} → (Γ : Cx S) → (a : Tm0) → a.size ≤ s →
  ...
  | s + 1, _, Γ, 𝛌 A b, h =>
      let f := fresh ((dom Γ, b) : Fset × Tm 1)
      have hΓ : f.val ∉ᶠ dom Γ := Fset.notMem_union_left f.property
      have hb : f.val # b := Fset.notMem_union_right f.property
      match synthLe s (Γ ⨟ f.val ∶ A ∣ hΓ) (b[f.val])
        (size_conc_le b f.val
          (Nat.le_trans (Nat.le_max_left _ _) (Nat.le_of_succ_le_succ h))) with
      | .no hn => .no fun p =>
          match p with
  ...
  | s + 1, _, Γ, b ∙ a, h =>
      ...
      | .yes ⟨C, p⟩ =>
        match synthLe s Γ a
          (Nat.le_trans (Nat.le_trans (Nat.le_max_left _ _) (Nat.le_max_right _ _))
            (Nat.le_of_succ_le_succ h)) with
        | .no hn => .no fun p =>
            match p with
            | ⟨_, .app _ p₁⟩ => hn ⟨_, p₁⟩
        | .yes ⟨A, p'⟩ =>
  ...
  | s + 1, _, Γ, 𝐬𝐮𝐜𝐜 a, h =>
      match decOfSynth 𝐍𝐚𝐭 (synthLe s Γ a
        (Nat.le_trans (Nat.le_max_left _ _) (Nat.le_of_succ_le_succ h))) with
      | .no hn => .no fun z =>
          match z with
          | ⟨_, .succ p⟩ => hn p
  ...
  | s + 1, _, Γ, 𝐧𝐫𝐞𝐜 c₀ cs a, h =>
      ...
      | .no hn => .no fun z =>
          ...
          | ⟨_, .nrec p _ _⟩ => hn ⟨_, p⟩
      | .yes ⟨C, p₀⟩ =>
        match decOfSynth (𝐍𝐚𝐭 ⇒ C ⇒ C) (synthLe s Γ cs
          (Nat.le_trans (Nat.le_trans (Nat.le_max_left _ _) (Nat.le_max_right _ _))
            (Nat.le_of_succ_le_succ h))) with
        | .no hn => .no fun z =>
            match z with
